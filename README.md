...
| 角色 | 功能 |
|------|------|
| 🛡️ 管理员 | 学院/管理员/教师/学生/课程 CRUD、操作日志审计、数据统计 |
| 👨‍🏫 教师 | 开设课程、停开课程、录入成绩 |
| 🎓 学生 | 选课/退课、查看成绩单与 GPA、修改密码 |

### 业务特性
- 📊 **仪表盘可视化** — Chart.js 柱状图/饼图/排行，按角色展示不同数据
- 🔍 **服务端搜索** — 跨页 SQL LIKE 搜索，不限于当前页
- 📄 **分页** — 所有列表 10 条/页，带页码导航
- 🔒 **重复选课拦截** — 前端+后端双重校验
- 👥 **选课人数上限** — 每门课最多 60 人
- 📝 **操作日志审计** — 记录所有关键操作，支持追溯
- 🎯 **GPA 计算** — 加权绩点 + 成绩等级（优秀/良好/中等/及格）
- 🔐 **密码修改** — 所有角色可修改密码
- 📦 **触发器/存储过程** — 学分上限拦截、课程停开校验
